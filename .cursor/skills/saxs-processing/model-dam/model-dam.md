...
This procedure wraps the `autosaxs model-dam` CLI command / `autosaxs.skill.model_dam` Python entry point.

## When to use me

- You want to run `autosaxs model-dam` on SAXS data.
...
2. Run **`/path/to/myenv/bin/autosaxs model-dam …`** (or `autosaxs model-dam …` when the right env is active), or call the Python function.
