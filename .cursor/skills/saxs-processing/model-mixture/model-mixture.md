...
This procedure wraps the `autosaxs model-mixture` CLI command / `autosaxs.skill.model_mixture` Python entry point.

## When to use me

- You want to run `autosaxs model-mixture` on SAXS data.
...
2. Run **`/path/to/myenv/bin/autosaxs model-mixture …`** (or `autosaxs model-mixture …` when the right env is active), or call the Python function.
