---
name: integrate-proxy
description: SAXS / small-angle x-ray scattering: integrate 2D TIFF image(s) to a 1D curve **without detector calibration**, using radial averaging in pixel-radius space (quick-look / debugging; not q-calibrated).
catalog-hidden: true
---
...
This skill wraps the `autosaxs integrate-proxy` CLI command / `autosaxs.skill.integrate_proxy` Python entry point.
