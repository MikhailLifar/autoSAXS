...
This procedure wraps the `autosaxs report-individual` CLI command / `autosaxs.skill.report_individual` Python entry point.

## When to use me

- You want to run `autosaxs report-individual` on SAXS data.
...
2. Run **`/path/to/myenv/bin/autosaxs report-individual …`** (or `autosaxs report-individual …` when the right env is active), or call the Python function.
