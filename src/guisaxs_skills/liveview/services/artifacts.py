...
def _newest_dammif_dummy_cif(subdir: Path) -> Optional[str]:
    from guisaxs_skills.modeling.catalogs.dammif_best import _newest_dammif_dummy_cif as _impl

    return _impl(subdir)


def best_dammif_cif(subdir: Path) -> Optional[str]:
    from guisaxs_skills.modeling.catalogs.dammif_best import best_dammif_cif as _impl
...
def discover_gnom_out_path(
    *,
    ...
    if fd.is_dir():
        for patt in (
            "gnom_best.out",
            "datgnom_best.out",  # legacy auto-path name (pre unified gnom_best.out)
            "datgnom_rg_*.out",
            "*_gnom.out",
            "*.out",
        ):
