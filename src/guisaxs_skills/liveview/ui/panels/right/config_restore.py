...
class RightPanelConfigRestore:
    def __init__(self, *, state: LiveviewSessionState) -> None:
        self._state = state
        self._model_mixture_saved_mixture_params: Optional[dict[str, Any]] = None

    @property
    def model_mixture_saved_mixture_params(self) -> Optional[dict[str, Any]]:
        return self._model_mixture_saved_mixture_params

    @property
    def fit_mixture_saved_mixture_params(self) -> Optional[dict[str, Any]]:
        """Deprecated alias for :attr:`model_mixture_saved_mixture_params`."""
        return self.model_mixture_saved_mixture_params
    ...
    def restore_bodies(self) -> None:
        if self._state.model_bodies_conf_path is not None:
            p = self._state.model_bodies_conf_path
            if p.is_file():
                self._ingest_bodies_conf_file(p)
            return
        for p in (
            self._state.watchdir / "model_bodies" / "model_bodies.conf",
            self._state.watchdir / "model_bodies" / "fit_bodies.conf",
            self._state.watchdir / "fit_bodies" / "fit_bodies.conf",
        ):
            if p.is_file():
                self._state.model_bodies_conf_path = p
                self._ingest_bodies_conf_file(p)
                return
    ...
    def restore_mixture(self) -> None:
        ...
        if live.is_file():
            self._state.model_mixture_config_path = live
            self._ingest_mixture_yaml(live)
            return
        p = self._state.model_mixture_config_path
        ...
        if side.is_file():
            ...
            if raw:
                ...
                if path.is_file():
                    self._state.model_mixture_config_path = path
    ...
    def _ingest_bodies_conf_file(self, path: Path) -> None:
        try:
            ...
            self._state.model_bodies_shapes = None
            return
        if not isinstance(data, dict):
            self._state.model_bodies_shapes = None
            return
        ...
        for x in sh:
            ...
            if t in BODIES_SHAPES_LIST and t not in out:
                out.append(t)
        self._state.model_bodies_shapes = out if out else None
    ...
    def _ingest_mixture_yaml(self, yml_path: Path) -> None:
        try:
            ...
            if not isinstance(data, dict):
                return
            m = data.get("model_mixture")
            if not isinstance(m, dict):
                m = data.get("fit_mixture")  # legacy section name
            if not isinstance(m, dict):
                m = data.get("mixture")
            if not isinstance(m, dict):
                return
            self._state.model_mixture_options = {str(k): v for k, v in m.items()}
            # Pane exposes max_nph, optional r_max/poly_max, optional q bounds.
            keys = ("max_nph", "r_max", "poly_max")
            self._model_mixture_saved_mixture_params = {k: m[k] for k in keys if k in m}
            wp = dict(self._state.polydisperse_window_params or {})
