...
def _subtract_paths(
    ...
    I_arr = np.asarray(I_sub, dtype=float)
    ax.plot(q_arr, I_arr, label="subtracted")
    if sigma_sub is not None:
        s_arr = np.asarray(sigma_sub, dtype=float)
        if s_arr.shape == I_arr.shape:
            ax.fill_between(q_arr, I_arr - s_arr, I_arr + s_arr, alpha=0.35)
