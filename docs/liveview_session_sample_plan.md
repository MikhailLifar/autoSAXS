...
| **Revision ingress** | `ingest/ingress.py` | `RevisionIngress.accept` — single front door for watcher/poll/tree/manual revisions. |

```text
Ingest backends → RevisionIngress → SampleStore / Executor
UI → LiveviewSession
...
| When | What |
|------|------|
| Enqueue (stable incoming) | `plan_for` full plan → queue (`Job.completed` empty) |
...
Backends (FLAT watcher, TIFF poller, TREE observer, aux dat, drops/Process) emit `SampleRevision` into **`RevisionIngress.accept`** → SampleStore boarding + executor enqueue. Watch topology follows `state.intake_mode`. Known-path baseline at watcher start; revisions via `FileStatSnapshot`.
