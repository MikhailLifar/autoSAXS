...
| **fit_distances** | Run ATSAS **GNOM** to obtain a pair distance distribution function \(p(r)\) for a **monodisperse** system from a 1D SAXS curve. The skill calls `gnom` from `PATH` in command-line mode, explicitly enforcing the monodisperse system type and scanning candidate `rmax` values to pick a robust solution. Input curves are typically in **nm⁻¹** and MUST be converted internally to ATSAS-compatible units (Å⁻¹) before calling GNOM. | `profile` (file/dir/glob of 1D `.dat`) | per-input lists of: `output_subdir`, `gnom_out_paths` (one per scanned rmax), `best_gnom_out_path`, `best_summary_path` |
