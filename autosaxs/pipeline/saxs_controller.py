...
PROMPTS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'resources', 'prompts')
DEBUG = True
...
class Controller:
    ...
    def get_descriptors(self, context: Context, to_analyze_path,
        ...
        if to_analyze_path:
            ...
            def _run_to_file(argv, out_path: str) -> None:
            ...
            if rg_val is not None:
                _run_to_file(
                    ["datgnom", atsas_dat_path, "-r", str(rg_val), "-o", gnom_file],
                    tmp_datgnom,
                )
