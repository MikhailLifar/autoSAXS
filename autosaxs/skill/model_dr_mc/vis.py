"""Plots for model_dr_mc (fit, D(R)±σ, optional McSAS result card)."""
...
def write_result_card_png(
    *,
    mcres: Any,
    out_path: str | Path,
) -> Tuple[Optional[str], Optional[str]]:
    """Best-effort McSAS3 result card. Returns (path, error)."""
    out_path = Path(out_path)
    try:
        from mcsas3.mc_plot import McPlot

        McPlot().resultCard(mcres, saveHistFile=out_path)
        if out_path.is_file() and out_path.stat().st_size > 0:
            return str(out_path), None
        return None, "McPlot.resultCard did not write a file"
...
def write_all_plots(result: Dict[str, Any], output_dir: str | Path, base: str) -> Dict[str, str]:
    ...
    card_path = output_dir / f"{base}_mcsas_result_card.png"
    ...
    card_written, card_err = write_result_card_png(mcres=result["mcres"], out_path=card_path)
    return {
        "fit_png_path": str(fit_path),
        "dr_png_path": str(dr_path),
        "result_card_png_path": card_written or "",
        "result_card_error": card_err or "",
    }
